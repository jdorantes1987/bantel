...
from accesos.datos import factura_venta_en_usd
from accesos.datos import articulos_profit
from accesos.datos import get_last__nro_fact_venta
...
l_col_art_conatel = ['art_146 2.3%', 'art_147 0.5%', 'art_148_anual 0.5%', 'art_150 1%', 'art_151 0.5%', 'Fonacit 0.5%']

def __doc_select_data(k_tabla, fec_ini, fec_fin):
    tabla = tabla_select[str(k_tabla).lower()]
    print(tabla)
    r_isrl_fini = to_datetime(date(to_datetime(fec_ini).year, to_datetime(fec_ini).month, 1),
                                 format='%d-%m-%Y').strftime('%Y%m%d')
    r_isrl_ffin = to_datetime(last_date_of_month(to_datetime(fec_ini).date()),
                                 format='%d-%m-%Y').strftime('%Y%m%d')
    sql = f"SELECT co_tipo_doc, iif(co_tipo_doc='N/CR', " \
    ...
    sql2 = f"SELECT co_tipo_doc, (total_bruto-monto_desc_glob) as base_imponible, monto_imp, total_neto, " \
           f"otros1 as IGTF, num_comprobante " \
           f"FROM {tabla} " \
    ...
    doc['co_tipo_doc'] = doc['co_tipo_doc'].str.strip()  # suprimir espacios en las cadenas de texto
    ...
    facturas = doc[doc['co_tipo_doc'].isin(docs_tipos) &
                   ~doc['num_comprobante'].str.contains('00000000')]  # Si no contiene comprobantes en cero
    ...
    if tabla == 'saDocumentoVenta':
        ...
        total_ant_islr = docs.loc[~docs['co_tipo_doc'].isin(['IVAN', 'ISLR']), 'base_imponible'].sum() * 0.01
        docs.loc['aislr'] = ['ANT. ISLR', total_ant_islr, 0, 0, total_ant_islr]
    docs = concat([docs, ret_islr_t], axis=0, ignore_index=True)
    docs.loc['tg'] = ['TOTAL NETO', docs.loc[
        ~docs['co_tipo_doc'].isin(['IVAN', 'ISLR', 'ANT. ISLR']), 'base_imponible'].sum(),
                      docs.loc[~docs['co_tipo_doc'].isin(['IVAN', 'ISLR', 'ANT. ISLR']), 'IGTF'].sum(),
                      docs.loc[~docs['co_tipo_doc'].isin(['IVAN', 'ISLR', 'ANT. ISLR']), 'monto_imp'].sum(),
                      docs.loc[~docs['co_tipo_doc'].isin(['IVAN', 'ISLR', 'ANT. ISLR']), 'total_neto'].sum()]
...
def datos_declaracion():
    ...
    ret_islr = df_union[df_union['Libro'].str.contains('c') & df_union['co_tipo_doc'].str.contains('ISLR')][
        'base_imponible'].tolist() if (dia_ini_qna > 15 or (dia_ini_qna == 1 and dia_fin_qna > 15)) else [0]
    l_union = [
        df_union[df_union['Libro'].str.contains('v') & df_union['co_tipo_doc'].str.contains('FACT')][
            'IGTF'].tolist(),
        df_union[df_union['Libro'].str.contains('c') & df_union['co_tipo_doc'].str.contains('IVAN')][
            'base_imponible'].tolist(),
        df_union[df_union['Libro'].str.contains('v') & df_union['co_tipo_doc'].str.contains('ANT. ISLR')][
            'base_imponible'].tolist(),
        ret_islr]
    df8 = DataFrame(l_union, columns=['Monto'], index=['IGTF VENTAS', 'RET. IVA',
                                                          'ANT. ISLR', 'RET. ISLR']).reset_index()
    ...
    df8['Monto$'] = df8['Monto$'].apply('${:,.2f}'.format)  # Se aplica formato de $ float
...
def graf_calor_ventas(usd=True):
    ...
    meses_letras = ('ENE', 'FEB', 'MAR', 'ABR', 'MAY', 'JUN', 'JUL', 'AGO', 'SEP', 'OCT', 'NOV', 'DIC')
    vtas['mes_letras'] =  vtas['mes'].apply(lambda x: meses_letras[x - 1])
    campos_monto_facturacion = ['monto_base_item', 'monto_base_item$']
    campo_select = campos_monto_facturacion[int(usd)] # Selecciona los montos en $ o Bs. ATENCIÓN: este codigo solo funciona con dos campos o dos valores lógicos ((1) VERDADERO (0) FALSO)
    print(vtas.to_string())
    # reshape flights dataeset in proper format to create seaborn heatmap
    flights_df = pivot_table(vtas, values=campo_select, index=['mes_letras'], columns=['anio'], aggfunc='sum', sort=False)
    print('\n' * 1, flights_df)
    fig, ax = subplots(figsize=(8, 7))
    # create seaborn heatmap, usar parametro center=True para colocar derivados de un color
    heatmap(flights_df, linewidths=0.5, annot=True, cmap='Blues', center=True, annot_kws={'size': 10},
                fmt=',.2f', cbar_kws={'format': '%.0f'})
...
def ajustes_aut_compras_sin_cta_contable():
    ...
    print('---compras')
    print(ajut_sin_cta[['co_tipo_doc', 'nro_doc', 'cob_num', 'fecha',
                        'total_bruto', 'dis_cen_y']].to_string(index=False))
...
def ajustes_man_compras_sin_cta_contable():
    ...
    print('---compras')
    print(ajut_sin_cta[['co_tipo_doc', 'nro_doc', 'observa', 'total_bruto',
                        'saldo', 'nro_orig', 'fec_reg', 'dis_cen']].to_string())
...
def ajustes_man_ventas_sin_cta_contable():
    ...
    ajut_sin_cta = ajut_man[ajut_man['dis_cen'].isnull()]  # Filtrar registros nulos
    print('---ventas')
    print(ajut_sin_cta[['co_tipo_doc', 'nro_doc', 'observa', 'total_bruto', 'saldo',
                        'nro_orig', 'fec_reg', 'dis_cen']].to_string())
...
def __get_impuestos_conatel__():
    ...
    datos_fact[l_col_art_conatel] = datos_fact.apply(lambda x: x[l_col_art_conatel] * x['monto_base_item'], axis=1)
...
def asiento_conatel(periodo):
    ...
    new_df2 = new_df.set_index(['anio', 'mes'])  # Asigna o establece 'anio' como columna clave
    ...
    ctas_gasto.append('6.1.15.01.0007')  # Fonacit Fondo Nacional para el Desarrollo del Deporte
    ctas_pasivo.append('2.4.01.07.0002')  # FIDETEL
    ctas_pasivo.append('2.4.01.07.0001')  # Fonacit Fondo Nacional para el Desarrollo del Deporte
    s = Series(list(new_df3.loc[periodo]), index=linea)  # Crea una serie para el dataframe de los gastos
    o_gasto = {'monto': s}
    ...
    s2 = Series(list(new_df3.loc[periodo]), index=linea2)  # Crea una serie para el dataframe de los pasivos
    o_pasivo = {'monto': s2}
    ...
    data_asiento_conatel = data_asiento_conatel[['cuenta', 'descrip_det', 'debe', 'haber', ]]  # Reordena las columnas
...
def facturacion_docs_sin_saldo():  # Facturas de ventas cobradas en su totalidad
    sql = f"SELECT RTRIM(co_tipo_doc) as co_tipo_doc, nro_doc, fec_reg, co_cli, " \
...
def facturacion_saldo_x_clientes_detallado(**kwargs):  # Facturas de ventas parcialmente cobradas o sin cobro
    print("\nFacturas con saldo a la fecha:")
    l_campos= campos_comunes_fact.copy()
    tasa = get_tasa()
    l_campos.extend(['saldo_total_doc', 'saldo_total_doc$', 'saldo_total_doc_actual$', 'dif_cambio$'])
    anio, mes, dato_cliente = kwargs.get('anio', 'all'), kwargs.get('mes', 'all'), kwargs.get('dato_cliente', 'all')
    df_fact = factura_venta_con_su_detalle_en_usd(anio=anio, mes=mes, usd=True)
    df_fact['saldo_total_doc_actual$']= df_fact.apply(lambda x: x['saldo_total_doc']/tasa, axis=1) 
    df_fact['dif_cambio$']= df_fact.apply(lambda x: x['saldo_total_doc$'] - x['saldo_total_doc_actual$'], axis=1) 
    df_fact_con_saldo = df_fact[df_fact['saldo_total_doc'] != 0][l_campos].drop_duplicates()
    datos_filter= df_fact_con_saldo
    if dato_cliente != 'all':
         datos_filter= df_fact_con_saldo[(df_fact_con_saldo['co_cli'].str.contains(dato_cliente)) | (df_fact_con_saldo['cli_des'].str.contains(dato_cliente))].copy()  # substring criteria 
    today = datetime.now()
    datos_filter["dias_transc"] = (today - datos_filter['fec_reg']).dt.days  # Dias transcurridos entre la ultima fecha al dia de hoy
    datos_filter = datos_filter.sort_values(by=['cli_des', 'saldo_total_doc', 'fec_reg'], ascending=[True, False, False])
    ...
    print('Saldo total $ según tasa actual:', round(datos_filter['saldo_total_doc_actual$'].sum(), ndigits=2))
    print('Saldo diferencia en cambio$:', round(datos_filter['dif_cambio$'].sum(), ndigits=2))
    return datos_filter

def facturacion_saldo_x_clientes_resumen(**kwargs):  # Facturas de ventas parcialmente cobradas o sin cobro
    ...
    pivot= pivot_table(df_fact, values=f_montos, index=['cli_des'], columns=['anio', 'mes'], aggfunc='sum', margins=True, sort=False).replace(nan, 0)  # Remplaza los valores nan con ceros
    return pivot

def facturas_cobradas_x_clientes_detallado(**kwargs):  # Facturas de ventas parcialmente cobradas o sin cobro
    ...
    df_fact['total_item_cobr'] = df_fact.apply(lambda x: x['total_item'] + x['igtf'] - x['saldo_total_doc'], axis=1) # Se le resta x['saldo_total_doc'] en caso de cobro parcial
    df_fact['total_item_cobr$'] = df_fact.apply(lambda x: x['total_item$'] + x['igtf$'] - x['saldo_total_doc$'], axis=1) # Se le resta x['saldo_total_doc$'] en caso de cobro parcial
    ...
    docts = df_fact_cobradas.sort_values(by=['cli_des', 'total_item_cobr', 'fec_reg'], ascending=[True, False, False])
    print('Total Bs.:', round(docts['total_item_cobr'].sum(), ndigits=2))
    print('Total $:', round(docts['total_item_cobr$'].sum(), ndigits=2))
    return docts

def facturas_cobradas_x_clientes_resumen(**kwargs):  # Facturas de ventas parcialmente cobradas o sin cobro
    anio, mes, dato_cliente, conv_usd = kwargs.get('anio', 'all'), kwargs.get('mes', 'all'), kwargs.get('dato_cliente', 'all'), kwargs.get('usd', True)
    ...
    pivot= pivot_table(df_fact, values=f_montos, index=['cli_des'], columns=['anio', 'mes'], aggfunc='sum', margins=True, sort=True).replace(nan, 0)  # Remplaza los valores nan con ceros
    return pivot

def diccionario_facturacion(**kwargs):  # Facturas de ventas parcialmente cobradas o sin cobro
    print("\nDiccionario de facturación general por año y mes:")
    anio, mes, conv_usd = kwargs.get('anio', 2023), kwargs.get('mes', 12), kwargs.get('conv_usd', False)
    print(anio, mes)
    df_fact = factura_venta_con_su_detalle_en_usd(anio=anio, mes=mes, usd=True)
    df_dic = pivot_table(df_fact, values=['monto_base_item', 'monto_base_item$'], index=['anio', 'mes_x'], aggfunc='sum', sort=True)
    a_usd = 0 if conv_usd == False else 1 
    anio_mes= round(df_dic.loc[(anio, mes)][a_usd], ndigits=2)
    return anio_mes

def diccionario_facturacion_total_por_anio(**kwargs):  # Facturas de ventas parcialmente cobradas o sin cobro
    print("\nDiccionario de facturación general por año:")
    anio, conv_usd = kwargs.get('anio', 2023), kwargs.get('conv_usd', False)
    print(anio)
    campos_monto_facturacion = ['monto_base_item', 'monto_base_item$']
    campo_select = campos_monto_facturacion[int(conv_usd)] # Selecciona los montos en $ o Bs. ATENCIÓN: este codigo solo funciona con dos campos o dos valores lógicos ((1) VERDADERO (0) FALSO)
    df_fact = facturacion_x_anio(conv_usd).groupby('anio')[[campo_select]].sum().reset_index()
...
def data_facturacion(**kwargs) -> DataFrame:
    ...
    data_['nro_control'] = str(doc_ctrol_emit).zfill(6) if format_fact == True else int(str(doc_ctrol_emit).replace('00-', ''))  # Rellena una cadena numérica con ceros a la izquierda
    data_['indice_fact'] = data_.apply(lambda x: x['id_client'] + str(x['enum']), axis=1)
    # asigna una numeracion a cada cliente que luego se usará para asignar el nuevo número de factura
    data_['grupo'] = factorize(data_['indice_fact'])[0] # factorize sirve para crear correlativo por grupos
...
def __data_encab_facturacion_masiva(indice_file, a_bs, num_doc, num_doc_ctrol, num_fact_format) -> DataFrame:

    data = data_facturacion(indice_file=indice_file, 
                            conv_a_bs=a_bs, 
                            num_doc=num_doc, 
    ...
    data_f = data.groupby(['id_client', 'nro_doc', 'nro_control', 'fecha_fact', 'descrip_encab_fact'])[
...
def facturacion_masiva(indice_file, a_bs, num_doc, num_doc_ctrol, num_fact_format):
    data = __data_encab_facturacion_masiva(indice_file, a_bs, num_doc, num_doc_ctrol, num_fact_format)
    # es necesario agrupar los encabezados de factura para totalizar la Base Imponible y el IVA
    data_agrupada = data.groupby(['id_client', 'nro_doc', 'nro_control', 'fecha_fact', 'descrip_encab_fact'])['monto_base'].sum().reset_index()
    ...
    data_iva = __determinar_impuesto_por_factura(indice_file, True, a_bs, num_doc, num_doc_ctrol, num_fact_format)
    ...
    for index, row in data_con_iva.iterrows():
        index += 1
        __exe_sql_insert_encab_facturacion(row["nro_doc"],
        ...
        procesar_det_facturacion(indice_file, row["nro_doc"], a_bs, num_doc, num_doc_ctrol, num_fact_format, suc)

 
 #  2 Filtra los datos del detalle de cada factura por la columna 'facturar'
def procesar_det_facturacion(indice_file, nro_documento, a_bs, num_doc, num_doc_ctrol, num_fact_format, sucursal):
    df = data_facturacion(indice_file=indice_file, 
                          conv_a_bs=a_bs, 
                          num_doc=num_doc, 
                          num_doc_ctrol=num_doc_ctrol,
                          num_fact_format=num_fact_format)
    ...
    data_iva_temp = __determinar_impuesto_por_factura(indice_file, False, a_bs, num_doc, num_doc_ctrol, num_fact_format)
    ...
    for index, row in data_con_iva.iterrows():
        ...
        __exe_sql_insert_det_facturacion(row["correl"],
                                         row["nro_doc"],
                                         row["co_art"],
...
def __determinar_impuesto_por_factura(indice_file, agrupado, a_bs, num_doc, num_doc_ctrol, num_fact_format):
    data_ = data_facturacion(indice_file=indice_file, 
                             conv_a_bs=a_bs, 
                             num_doc=num_doc, 
                             num_doc_ctrol=num_doc_ctrol,
                             num_fact_format=num_fact_format)
    ...
    if agrupado == True:
        # es necesario agrupar los encabezados de factura para totalizar la Base Imponible y el IVA
        data_agrupada = merg1.groupby(['id_client', 'nro_doc', 'fecha_fact', 'descrip_encab_fact', 'tipo_imp', 'p_iva'])[
...
def __exe_sql_insert_encab_facturacion(id_doc, descrip, cod_cli, vendedor, fecha_fact, fecha_cur, monto_base, monto_iva,
...
def __exe_sql_insert_det_facturacion(linea, id_doc, co_art, total_item, monto_iva, fecha_cur, cant_art, total_reng,
