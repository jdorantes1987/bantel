...
from administracion_profit.edo_cta import get_mov_edo_cta_pdtes_por_regist
from administracion_profit.edo_cta import get_mov_bcarios_pdtes_por_identificar
...
def registrar_mov_ban_edo_cta():
    ...
    for ind in df_edo_cta2.index:
        if str(sheet.cell(row=[ind][0] + 2, column=10).value).upper() == 'SI':
            ...
            add_new_mov(id_mov_bco, cell_coll_descipc, cell_coll_cta_ie, cell_coll_fecha, cell_coll_ref, cell_coll_monto, cell_coll_idb)
            cell_print_result.value = 'CONTABILIZACIÓN-->' + cell_coll_cta_ie + '->' + str(id_mov_bco)
...
def mov_bcarios_pendientes_por_identif_en_edo_cta_banesco(fecha_ini):
    ...
    movimientos = get_movbanco(fecha_ini)  # Obtiene los movimientos bancarios del mes especificado
    ...
    mov_pdtes_banesco = mov_pdtes[
                            mov_pdtes['cod_cta'] == '0134  '
                        ].copy()
    mov_pdtes_banesco['descrip'] = mov_pdtes_banesco['descrip'].str[:40]
    return mov_pdtes_banesco[['mov_num', 'fecha', 'descrip', 'co_cta_ingr_egr', 'doc_num',
                              'monto_d', 'monto_h', 'cob_pag', 'co_us_in', 'fe_us_in', 'cod_cta']]
