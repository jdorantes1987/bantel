...
import numpy as np
from pandas import DataFrame, concat, merge, read_excel, to_datetime
...
def get_mov_igtf_comisiones():
    data_edo_cta = read_excel(p_edo_cta_banesco, dtype={"Referencia": str}).replace(
        np.nan, ""
    )
    f_edo_cta_sin_nulos = data_edo_cta[
        (data_edo_cta["Fecha"].notnull()) & (data_edo_cta["Fecha"] != "")
...
def __get_asiento_contable(descripcion_asiento):
    ...
    # cuentas de gasto
    union["Debe"] = union.apply(
        lambda x: abs(x["Monto"]) if x["Cuenta_contable"] in set(l_ctas_gasto) else 0.0,
        axis=1,
    )
    union["Haber"] = union.apply(
        lambda x: (
            abs(x["Monto"]) if x["Cuenta_contable"] not in set(l_ctas_gasto) else 0.0
        ),
        axis=1,
    )
    union["Descripcion"] = descripcion_asiento
    union["Referencia"] = union["Referencia"].astype("Int64")
    union[["Cuenta_contable", "Referencia", "Descripcion", "Debe", "Haber"]].to_excel(
        "union.xlsx"
    )
...
def contabilizar_comisiones_e_igtf(id_cbte, fecha_cbte, descripcion_cbte):
    date_cbte = to_datetime(
        date(
            to_datetime(fecha_cbte).year,
            to_datetime(fecha_cbte).month,
    ...
    cbte.new_encab_comprobante(
        id_cbte, descripcion_cbte, date_cbte, date_current, date_current
    )
    print("Insertando...")
