...
def search_prov(string_s):
    df = proveedores().copy()
    df['direc1'] = df['direc1'].str[:60]  # Extrae los primeros 60 caracteres de la izquierda
    df['prov_des'] = df['prov_des'].str[:45]  # Extrae los primeros 60 caracteres de la izquierda
    df['fe_us_in'] = pd.to_datetime(df['fe_us_in']).dt.normalize()
    resul = busq_in_dataframe(string_s, df)[['co_prov', 'prov_des', 'rif', 'telefonos', 'respons', 'direc1', 'co_us_in']]
    return resul
...
def get_top_compras_x_prov(top=15):
    ...
    result = data_fact.nlargest(n=top, columns=['monto_base_item'], keep='all').reset_index(drop=True)
    join1 = pd.merge(result, prov, how='left', left_on='co_prov', right_on='co_prov')
    join1['co_prov'] = join1['co_prov'].str.strip()  # Elimina o suprime los espacios de más
    result = join1[l_col]
