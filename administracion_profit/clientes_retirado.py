...
from accesos.datos import clientes, factura_venta_con_su_detalle_en_usd
...
def clientes_retirados():
    ...
    ult_fact_df = (
        df2.groupby(["co_cli"])
        .agg({"fec_reg": "max", "anio": "count", "doc_num": "max"})
        .reset_index()
    )
    group_fact_df = (
        df2.groupby(["doc_num"]).agg({"monto_base_item$": "sum"}).reset_index()
    )
    ...
    ult_fact_df["meses_sin_fact"] = ult_fact_df["fecha_final"].dt.to_period("M").astype(
        "int64"
    ) - ult_fact_df["fec_reg"].dt.to_period("M").astype("int64")
    # filtra aquellas facturas cuyos meses sin facturas estén entre 3 y 12 mese y por lo menos se le emitieron dos facturas
    segm_mes = ult_fact_df[
        (ult_fact_df["meses_sin_fact"] >= 1)
        & (ult_fact_df["meses_sin_fact"] <= 12)
        & (ult_fact_df["anio"] > 1)
    ]
    merge_fact = merge(segm_mes, group_fact_df, how="left", on="doc_num")
    cliente = clientes()[["co_cli", "cli_des", "inactivo"]]
    merge_fact2 = merge(merge_fact, cliente, how="left", on="co_cli")[
        [
            "co_cli",
            "cli_des",
            "fec_reg",
    ...
    print(
        "Total base imponible: $ {:,.2f}".format(merge_sort["monto_base_item$"].sum())
    )
    return merge_sort
...
def graf1():
    ...
    plots.tick_params(
        axis="x", labelrotation=90, labelsize=7.0
    )  # Rotación eje de las x
    plt.xlabel("Clientes", size=12)
    plt.ylabel("Monto facturación mes", size=12)
    plt.title(
        "Impacto de los clientes retirados sobre la facturación \n Total impacto: $ {:,.2f}".format(
            df["monto_base_item$"].sum()
        )
    )
