...
from pandas import DataFrame, concat, merge_asof, read_excel, to_datetime
...
def get_edo_cta_bs_y_usd():
    ...
    edo_cta_sort = edo_cta.sort_values(
        by=["Fecha"], ascending=[True]
    )  # se debe ordenar el df para poder conbinar
    ...
    merge_data["Comentarios"] = merge_data["Comentarios"].str[
        :60
    ]  # Extrae los primeros 60 caracteres de la izquierda
    merge_data_sort = merge_data.sort_values(
        by=["Fecha", "Referencia", "Monto"], ascending=[False, True, True]
    )
...
def get_mov_edo_cta_pdtes_por_regist(str_date):
    mov_sin_registrar = conjunto_ref_bcarias_edo_cta() - conjunto_ref_mov_bcrios(
        str_date
    )
...
def get_mov_bcarios_pdtes_por_identificar(str_date):
    mov_sin_registrar = (
        conjunto_ref_mov_bcrios(str_date) - conjunto_ref_bcarias_edo_cta()
    )
    return mov_sin_registrar
