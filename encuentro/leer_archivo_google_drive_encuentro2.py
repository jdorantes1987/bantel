...
df = pd.concat(
    pd.read_excel(file_name, sheet_name=["ENCUENTRISTAS", "SERVIDORES"]),
    ignore_index=True,
)
...
p_x_tipo.loc["T"] = [
    "TOTAL",
    "",
    p_x_tipo["MONTO ABONADO"].sum(),
    p_x_tipo["MONTO X ABONAR"].sum(),
]
...
campos_file = [
    ["NOMBRE", "TIPO", "SEXO", "RED", "MONTO ABONADO", "MONTO X ABONAR", "ENVIADO POR"],
    ["TIPO", "SEXO", "RED"],
    [True, True, True],
]
...
solv_s = solv_l.sort_values(by=campos_file[1], ascending=campos_file[2]).reset_index(
    drop=True
)
...
abonos_s = abonos_l.sort_values(
    by=campos_file[1], ascending=campos_file[2]
...
s_pagos_s = s_pagos_l.sort_values(
    by=campos_file[1], ascending=campos_file[2]
).reset_index(drop=True)
