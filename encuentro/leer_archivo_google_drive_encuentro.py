...
print('\n' * 2, 'Finanzas')
fnzas = df.groupby(['SEXO']).agg({'MONTO ABONADO': 'sum', 'MONTO X ABONAR': 'sum'}).reset_index()
...
for ind in pptes_solventes.index:
    print(pptes_solventes['NOMBRE'][ind], pptes_solventes['IGLESIA'][ind], pptes_solventes['RED'][ind], sep='--')
...
tep = env_por.sort_values(by=['RED', 'CANTIDAD'], ascending=[True, False])  # Ordenar por varias columnas
print('\n' * 2, 'Total:')
...
print('\n' * 2, 'Edad promedio por:')
prom_edades = df.groupby(['SEXO'])['EDAD'].aggregate(['mean', 'max', 'min']).reset_index()
prom_edades['mean'] = prom_edades['mean'].apply(lambda x: int(round(x, 0)))
prom_edades.rename(columns={'mean': 'EDAD_PROMEDIO', 'max': 'EDAD_MAS_ALTA', 'min': 'EDAD_MAS_BAJA'}, inplace=True)
