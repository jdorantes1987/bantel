...
path_file_articulos = 'F:/ESTRUCTURA-INGRESOS/Admin-Izq-Estructura-Ingresos-Articulos.xlsm'
...
def agrupa_facturacion(data_linea_art):
    l_campos = ['co_base', 'grupo', 'descripcion_cod_base', 'monto', 'es_detalle', 'sum_x_grupo', 'porce_x_grupo',
                'descripcion_grupo', 'g1', 'g2', 'g3', 'g4', 'g5', 'g6']
    ...
    linea_art['grupo'] = linea_art.apply(lambda x: str(x['co_lin'])[:5] if x['grupo'] == 'nan' else x['grupo'], axis=1)
    join_ph = pd.merge(linea_art, linea_art, left_on='grupo', right_on='co_lin', suffixes=('_p', '_h'))
    join1 = pd.merge(linea_art, join_ph, left_on='co_lin', right_on='co_lin_p')
    join1 = join1.rename(columns={'co_lin': 'co_base',
                                  'detalle': 'es_detalle',
                                  'lin_des': 'descripcion_cod_base',
                                  'lin_des_h': 'descripcion_grupo'})

    join1['sum_x_grupo'] = join1.apply(
        lambda x: join1[(join1['co_base'].str[:len(x['co_base'])] == x['co_base'])]['monto'].sum()
        if x['es_detalle'] == 0 else x['monto'], axis=1)

    join1['porce_x_grupo'] = join1.apply(
        lambda x: x['monto'] / join1[join1['grupo'] == x['grupo']]['monto'].sum()
        if x['es_detalle'] == 1 and x['sum_x_grupo'] != 0 else
        (x['sum_x_grupo'] / join1[(join1['co_base'] == x['grupo'])]['sum_x_grupo'].sum())
        if (x['sum_x_grupo'] != 0) and len(x['co_base']) != 5 else x['sum_x_grupo'] / join1['monto'].sum()
        if x['sum_x_grupo'] != 0 else 0, axis=1)
    ...
    join1['g1'] = join1['co_base'].apply(lambda x: linea_art_index.loc[x[:3], 'lin_des'])
    join1['g2'] = join1['co_base'].apply(lambda x: linea_art_index.loc[x[:6], 'lin_des'])
    join1['g3'] = join1['co_base'].apply(lambda x: linea_art_index.loc[x[:8], 'lin_des'])
    join1['g4'] = join1['co_base'].apply(lambda x: linea_art_index.loc[x[:10],'lin_des'] if len(x)>=10 else None)
    join1['g5'] = join1['co_base'].apply(lambda x: linea_art_index.loc[x[:12], 'lin_des'] if len(x)>=12 else None) 
    join1['g6'] = join1['co_base'].apply(lambda x: linea_art_index.loc[x[:16], 'lin_des'] if len(x)>=16 else None) 
