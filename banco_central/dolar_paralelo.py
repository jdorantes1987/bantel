...
def bajar_data_dolar_par():
    ...
    for i in range(1, 2, 1):
        ...
        all_tables = pd.read_html(response.text, header=0)  # header=0 indica que la lista tiene encabezados
        response.close()
        df_arr_pg.append(all_tables[0])
        print('se descargo:', url)
    df = pd.concat(df_arr_pg, axis=0, ignore_index=True)
    data = df.rename(columns={'Fecha': 'fecha',
                              'Precio': 'venta_ask2'})  # Renombrar o cambiar nombres a las columnas
    data['var_tasas'] = data['venta_ask2'].diff(periods=-1)  # Permite calcular la diferencia que existe entre el valor de la celda actual con respecto a la anterior
...
def get_data_usd_par_web_last_qt():
    ...
    data['var_tasas'] = data['venta_ask2'].diff(
        periods=-1)
    data['fecha'] = pd.to_datetime(data['fecha'], dayfirst=True)  # convierte a fecha formato datetime
    return data
...
def actulizar_file_tasas():
    ...
    dolar_par_max_diario['fecha'] = dolar_par_max_diario.index  # Convierte los valores de los indices a columna
    dolar_par_max_diario = dolar_par_max_diario[['fecha', 'venta_ask2', 'var_tasas', 'año', 'mes', 'dia', 'mes_']]  # Reordenación de columnas
...
def datos_estadisticas_tasas():
    ...
    hist_tc = hist_tc.drop(hist_tc.columns[0], axis=1).reset_index(
        drop=True)  # Elimina la primera columna del dataframe
...
def preparar_file_estadisticas_tasas():
    ...
    dolar_par_max_diario['fecha'] = dolar_par_max_diario.index  # Convierte los valores de los indices a columna
    dolar_par_max_diario = dolar_par_max_diario[['fecha', 'venta_ask2']]  # Reordenación de columnas
    data = dolar_par_max_diario.reset_index(drop=True)
    data_sort = data.sort_values(by=['fecha'], ascending=[False]).reset_index(drop=True)
    # Permite calcular la diferencia que existe entre el valor de la celda actual con respecto a la anterior
