...
def facturacion_notas_credito(**kwargs):
    l_campos = ['reng_num', 'doc_num', 'co_art', 'cod_art_izq', 'co_tipo_doc', 'fec_emis', 'fec_reg', 'fec_venc', 'anio', 'mes',
                'co_ven', 'co_cli', 'cli_des', 'descrip', 'monto_base_item', 'iva', 'igtf', 'total_item', 'saldo_total_doc']
    anio, mes = kwargs.get('anio'), kwargs.get('mes')
...
l_campos_facturacion = ['reng_num', 'doc_num', 'co_art', 'cod_art_izq', 'co_tipo_doc', 'fec_emis', 'fec_reg', 'fec_venc', 'anio', 'mes_x',
                        'co_ven', 'co_cli', 'cli_des', 'descrip', 'monto_base_item', 'iva', 'igtf', 'total_item', 
                        'saldo_total_doc', 'monto_base_item$', 'iva_usd$', 'total_item$', 'saldo_total_doc$','igtf$']
...
def factura_venta_con_su_detalle_en_usd(**kwargs):
    ...
    fact_detalle_sort = fact_detalle_total.sort_values(by=['fec_reg'],
                                                       ascending=[True])  # se debe ordenar el df para poder conbinar
    data_bcv_sort = df_data_bcv.sort_values(by=['fecha'],
                                            ascending=[True])  # se debe ordenar el df para poder conbinar
    merge_data = merge_asof(fact_detalle_sort, data_bcv_sort, left_on='fec_reg', right_on='fecha',
                            direction="nearest")  # Combinar por aproximación
    merge_data['igtf$'] = merge_data.apply(lambda x: x['igtf'] / x['venta_ask2'], axis=1)
    ...
    merge_data['iva_usd$'] = merge_data.apply(lambda x: x['iva'] / x['venta_ask2'] if conv_usd else x['iva'], axis=1)
    merge_data['total_item$'] = merge_data.apply(lambda x: x['total_item'] / x['venta_ask2'] if conv_usd and x['reng_num'] == 1 else 0, axis=1)
    merge_data['saldo_total_doc'] = merge_data.apply(lambda x: x['saldo_total_doc'] if x['reng_num'] == 1  else 0, axis=1)
    merge_data['saldo_total_doc$'] = merge_data.apply(lambda x: x['saldo_total_doc'] / x['venta_ask2'] if conv_usd and x['reng_num'] == 1  else 0, axis=1)
...
def factura_venta_en_usd(**kwargs):
    anio, mes, conv_usd = kwargs.get('anio', 'all'), kwargs.get('mes', 'all'), kwargs.get('usd', True)
    l_campos= l_campos_facturacion.copy()
    df_fact = factura_venta_con_su_detalle_en_usd(anio=anio, mes=mes, usd=conv_usd).copy()
...
def facturacion_x_anio(usd=False):
    ...
    fact_x_periodos = doc_g2.sort_values(by=['anio', 'mes', campo_select],
...
def get_id_movbanco():
    df = get_read_sql("Select mov_num from saMovimientoBanco "
                      "where origen='BAN'", **dict_con_admin)
    mb = df['mov_num'].max()
...
def search_in_compras(**kwargs) -> DataFrame:
    ...
    merge_data['USD_BASE'] = merge_data['USD_BASE'].apply('${:,.2f}'.format)  # Se aplica formato de $ float
    merge_data['USD_IVA'] = merge_data['USD_IVA'].apply('${:,.2f}'.format)  # Se aplica formato de $ float
    merge_data['USD_TOTAL'] = merge_data['USD_TOTAL'].apply('${:,.2f}'.format)  # Se aplica formato de $ float
    merge_data['USD_IGTF'] = merge_data['USD_IGTF'].apply('${:,.2f}'.format)  # Se aplica formato de $ float
