...
def conexion(**kwargs):
    ...
    con_str = f'DRIVER={driver_con};SERVER={serv};DATABASE={data_base};UID={user};PWD={pword}'
...
def insert_sql(strsql, **kwargs):
    ...
    pword = args['pword']
    str_conn = "DRIVER={prov};SERVER={host};DATABASE={db};UID={user};PWD={pw}".format(prov=proveedor, host=serv,
                                                                                      db=data_b, user=usuario, pw=pword)
