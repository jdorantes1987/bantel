...
from pandas import read_sql_query, NA
...
class ConexionBDMysql(ConexionBD):
    def __init__(self, **kwargs):
        super().__init__(**{k: kwargs.get(k, v) for k, v in {
            "proveedor": "{MySQLdb}",
        ...
        self.url_conexion = f"mysql+mysqlconnector://{self.usuario}:{self.clave}@{self.servidor}/" \
                            f"{self.bddatos}?auth_plugin=mysql_native_password"

    def conectar(self):
        try:
            self.conn = pymysql.connect(host=self.servidor,
                                        user=self.usuario,
                                        password=self.clave,
