...
from accesos.datos import dict_con_contab, get_identificador_unicos
...
def get_comprobantes(**kwargs):
    ...
    strsql = (
        "Select reng_num, fec_emis, descri, co_cue, RTRIM(docref) as docref, monto_d, monto_h "
        "From scren_co "
        "Where co_cue = '{cta}' and "
        "fec_emis >= '{f_desde}' and "
        "fec_emis <= '{f_hasta}'".format(
            cta=cuenta, f_desde=fecha_desde, f_hasta=fecha_hasta
        )
    )
...
def new_encab_comprobante(id_cbte, descrip, fecha_emision, fecha_insert, fecha_mod):
    strsql = (
        "INSERT sccompro (comp_num, fec_emis, tipo, procesado, descrip, origen, inte_num, inte_num2, "
        ...
        "NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL)".format(
            id_Cbte=id_cbte,
            fech_emi=fecha_emision,
            descr=descrip,
            fech_ins=fecha_insert,
            fech_mod=fecha_mod,
        )
    )
...
def new_line_comprobante(
    ...
    strsql = (
        "INSERT scren_co (comp_num, fec_emis, reng_num, monto_h, monto_d, descri, docref, fec_doc, "
