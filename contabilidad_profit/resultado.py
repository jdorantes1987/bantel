...
from pandas import Categorical, DataFrame, melt, pivot_table, read_excel
...
def unpivot_resultado_mes():
    ...
    unpivot = melt(
        df, value_vars=columnas
    )  # Convierte las columnas de meses a filas con su valor asociado
...
def resultado_graf_map_calor():
    ...
    flights_df = pivot_table(
        resultado, values="sum", observed=True, index=["mes"], aggfunc="sum", sort=False
    )
