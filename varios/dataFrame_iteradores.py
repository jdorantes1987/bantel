...
data3 = {
    "Name": ["Jackson", "Amit", "Aishwarya", "Priyanka"],
    "Age": [21, 19, 20, 18],
...
# Definir un diccionario que contenga datos de estudiantes
data2 = {
    "Name": ["Jakson", "Amit", "Aishwarya", "Priyanka"],
    "Age": [21, 19, 20, 18],
...
print(df2)
print(
    "\nUnicos", df2[~df2["Percentage"].duplicated(keep=False)]
)  # keep=False permite identificar todos los duplicados, ~ niega los valores boleanos.
print(
    "\nDuplicados", df2[df2["Percentage"].duplicated(keep=False)]
)  # keep=False permite identificar todos los duplicados, tiene varias opciones, leer documentación.
...
# Columnas 'Nombre' y 'Porcentaje' respectivamente.
df6["prueba"] = df6.apply(
    lambda df_row: df_row["Name"] + " " + str(df_row["Percentage"]), axis=1
)
print(df6)
