from pandas import merge_asof, read_excel
...
join1 = merge_asof(
    pagos_s, tasas_cambio_s, left_on="fecha", right_on="fecha", direction="nearest"
)  # Combinar por aproximación
merge_data = join1.sort_values(by=["cob_num", "reng_doc"], ascending=[True, True])
merge_data[l_campos_sin_espacios] = merge_data[l_campos_sin_espacios].apply(
    lambda x: x[l_campos_sin_espacios].str.strip(), axis=1
)
merge_data[l_campos_a_recortar] = merge_data[l_campos_a_recortar].apply(
    lambda x: x[l_campos_a_recortar].str[:40], axis=1
)
merge_data["monto_usd"] = merge_data.apply(
    lambda x: round(x["mont_doc"] / x["venta_ask2"], ndigits=2), axis=1
)
print(merge_data[l_campos].reset_index(drop=True).to_string())
print(
    "Cantidad de nombres únicos:", len(merge_data["prov_des"].unique().tolist())
)  # lista de unicos
